...
/* Full-screen image carousel for a flyer album. Navigation is manual only
   (arrow buttons, dots, swipe/drag, or left/right keys) — no auto-advance.
   On open, a few preview panels fan out from the stacked "album" look and
   settle into the carousel, instead of the dialog just popping in static. */
export function FlyerLightbox({ open, images = [], name, onClose }) {
  ...
  function handleDragEnd(x) {
    if (typeof dragStartX.current !== 'number') return;
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200,
      ...
      <div role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}
        ...
        <div
          ...
          {/* fan-out intro: mirrors AlbumStack's stacked look, then spreads and fades as the carousel settles in */}
          {Array.from({ length: fanCount }).map((_, i) => {
            ...
            return (
              <div key={i} aria-hidden="true" style={{ position: 'absolute', width: '55%', maxWidth: 320, aspectRatio: '3 / 4',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%',
            ...
            <img src={images[index]} alt={`${name} ${index + 1} of ${count}`} draggable={false}
