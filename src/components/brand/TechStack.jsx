...
import { siAstro, siHtml5, siJavascript, siPhp, siLaravel, siReact, siMysql, siPostgresql, siGit, siNpm, siComposer, siWordpress } from 'simple-icons/icons';
import css3Raw from 'devicon/icons/css3/css3-original.svg?raw';
import photoshopRaw from 'devicon/icons/photoshop/photoshop-original.svg?raw';
import illustratorRaw from 'devicon/icons/illustrator/illustrator-original.svg?raw';
...
const photoshopSvg = photoshopRaw.replace('<svg ', '<svg width="100%" height="100%" ');
const illustratorSvg = illustratorRaw.replace('<svg ', '<svg width="100%" height="100%" ');
const vscodeSvg = vscodeRaw.replace('<svg ', '<svg width="100%" height="100%" ');
const CSS3_HEX = '1572B6'; // official W3C CSS3 badge blue, used for the hover accent to match
const PHOTOSHOP_HEX = '31A8FF';
const ILLUSTRATOR_HEX = 'FF9A00';
const VSCODE_HEX = '007ACC';
const ARTISAN_HEX = 'FF2D20'; // Laravel red — Artisan has no brand mark of its own, so a terminal glyph stands in for it
...
function TechDialog({ item, onClose }) {
  ...
  return (
    <dialog ref={ref} className="ctech-dialog" onClose={onClose} aria-label={item ? `${item.name} details` : undefined}
...
function StackCard({ icon, customSvg, LucideIcon, hex, name, spin, onOpen }) {
  const [hover, setHover] = React.useState(false);
  const rgb = hexToRgb(hex);
  const accent = `#${hex}`;
  return (
    <button type="button" style={{ ...cardStyle, width: '100%', fontFamily: 'inherit',
      border: '1px solid ' + (hover ? `rgba(${rgb}, 0.6)` : 'var(--space-border)'),
      ...
      <div style={spin ? { width: 30, height: 30, animation: 'ctech-spin 4.375s linear infinite' } : { width: 30, height: 30 }}>
        {LucideIcon
          ? <LucideIcon size={30} strokeWidth={1.5} color={hover ? accent : 'var(--text-secondary)'} />
          : customSvg
            ? <div style={{ width: 30, height: 30 }} dangerouslySetInnerHTML={{ __html: customSvg }} />
            : <TechIcon icon={icon} />}
      </div>
      <div style={{ fontSize: 14, fontWeight: 600,
        color: hover ? accent : 'var(--text-secondary)',
        textShadow: hover ? `0 0 14px rgba(${rgb}, 0.5)` : 'none',
        transition: 'color 200ms var(--ease-hud), text-shadow 200ms var(--ease-hud)' }}>{name}</div>
    </button>
  );
}
...
function AstroStackCard({ onOpen }) {
  return (
    <button type="button" style={{ ...cardStyle, width: '100%', fontFamily: 'inherit', border: '1px solid var(--nebula-cyan-30)', boxShadow: 'var(--glow-cyan-soft)' }}
      aria-label="Astro details" onClick={onOpen}>
...
const stack = [
  ...
  { icon: siNpm, hex: siNpm.hex, name: 'npm', category: 'Tool',
    description: 'The package manager used to install and manage the JavaScript libraries and tools behind a project.' },
  { icon: siComposer, hex: siComposer.hex, name: 'Composer', category: 'Tool',
    description: "PHP’s dependency manager, used to install and manage the libraries behind a Laravel or PHP project." },
  { LucideIcon: Terminal, hex: ARTISAN_HEX, name: 'Artisan', category: 'Tool',
    description: "Laravel’s command-line tool, used to scaffold code, run database migrations, and automate backend tasks." },
  // CMS — always last: not part of the default stack
  { icon: siWordpress, hex: siWordpress.hex, name: 'WordPress', category: 'CMS',
    description: 'A content management system. Not part of our default stack, only used when a client specifically requests it.' },
];

const designTools = [
  { customSvg: photoshopSvg, hex: PHOTOSHOP_HEX, name: 'Photoshop', category: 'Design Tool',
    description: 'Used for photo editing, image compositing, and print-ready artwork like flyers and posters.' },
  { customSvg: illustratorSvg, hex: ILLUSTRATOR_HEX, name: 'Illustrator', category: 'Design Tool',
    description: 'Used for vector artwork like logos, icons, and print layouts that need to scale cleanly at any size.' },
  { customSvg: vscodeSvg, hex: VSCODE_HEX, name: 'VS Code', category: 'Editor',
    description: 'The code editor used to write and maintain every project.' },
];

function StackGrid({ items, includeAstro }) {
  const [selected, setSelected] = React.useState(null);
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 16 }}>
        {includeAstro && <AstroStackCard onOpen={() => setSelected(astroItem)} />}
        {items.map((t) => (
          <StackCard key={t.name} icon={t.icon} customSvg={t.customSvg} LucideIcon={t.LucideIcon} hex={t.hex} name={t.name} spin={t.spin}
...
export function TechStack() {
  return <StackGrid items={stack} includeAstro />;
}

export function DesignToolsStack() {
  return <StackGrid items={designTools} includeAstro={false} />;
}
