...
const ControlButton = React.forwardRef(function ControlButton({ label, onClick, style, children }, ref) {
  const [hover, setHover] = React.useState(false);
  return (
    <button ref={ref} aria-label={label} title={label} onClick={onClick}
      ...
      style={{ width: 40, height: 40, display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
        ...
        transition: 'border-color 200ms var(--ease-hud), background 200ms var(--ease-hud)', ...style }}>
      {children}
    </button>
  );
});
...
export function ImageLightbox({ open, images = [], name, aspectRatio = '3 / 4', onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200,
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} onClick={(e) => e.stopPropagation()}
        style={{ position: 'relative', width: '100%', maxWidth: 720, display: 'flex',
          flexDirection: 'column', alignItems: 'center', gap: 16 }}>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <div id={titleId} style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>{name}</div>
          <ControlButton label="Close" onClick={onClose} ref={closeBtnRef}>
        ...
        {multi && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {images.map((_, i) => (
              <button key={i} aria-label={`Go to image ${i + 1}`} aria-current={i === index ? 'true' : undefined} onClick={() => setIndex(i)}
                style={{ width: 7, height: 7, borderRadius: '50%', padding: 0, border: 'none', cursor: 'pointer',
                  ...
                  transition: 'background 200ms var(--ease-hud), box-shadow 200ms var(--ease-hud)' }} />
            ))}
          </div>
        )}
      </div>
