...
import { TechBadgeRow } from './TechBadges.jsx';
...
function ConceptCard({ concept, onOpen, hidden }) {
  return (
    <button type="button" onClick={() => onOpen(concept)} tabIndex={hidden ? -1 : 0} aria-hidden={hidden || undefined}
      style={{ position: 'relative', width: 240, flexShrink: 0, cursor: 'pointer',
        display: 'block', textAlign: 'left', font: 'inherit', color: 'inherit', background: 'none', border: 'none', padding: 0 }}>
      <div style={{ position: 'relative', aspectRatio: '4 / 3', background: 'var(--space-surface)',
        ...
        <TechBadgeRow techs={concept.tech} offset={24} />
        {concept.image ? (
...
export function ConceptMarquee({ items = [], onOpen, forcePaused = false }) {
  ...
  const paused = hovered || focused || forcePaused;
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: 'flex', gap: 24, width: 'max-content',
        ...
        {track.map((c, i) => <ConceptCard key={`${c.name}-${i}`} concept={c} onOpen={onOpen} hidden={i >= items.length} />)}
      </div>
    </div>
  );
}
