...
/* Fanned photo-pile look for multi-image flyer projects. Renders up to 3 layers;
   ...
   taller than its box, however wide the grid track ends up being. Each album can
   pass its own aspectRatio so the card matches its actual flyer proportions
   instead of forcing every album into the same shape. */
export function AlbumStack({ images = [], aspectRatio = '3 / 4' }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 220, margin: '0 auto', aspectRatio }}>
      {layers.map((img, i) => {
